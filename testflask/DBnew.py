...
def user_display():
    user = db.user
    a = user.find({"activate":1},projection={"_id":0})
    b = list(a)
    return jsonify(b)
...
def add():
    ...
    try:
        panel_id = PANEL.insert({'Barcode' : info['barcode'], 'cell_type': info['cell_type'],'cell_amount': info['cell_amount'],'module_no':info['module_no'],'EL_no':info['el_no'],'create_time':info['create_time']})
    except BaseException as e:
